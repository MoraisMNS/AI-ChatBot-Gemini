...
class ChatBot:
    ...
    def _construcr_message(self,text,role='user'):
        return{
            'role' : role,
            'parts': [text]
        }    
        ...
        if isinstance(conversation_history,list):
            self._conversation_history = conversation_history
                ...
                self._conversation_history=[
                   self._construcr_message('From now on,return the output as a JSON object that can be loaded in Python with the key as \'text\'.For example,{"text":"<output goes here>"}'),
                   self._construcr_message('{"text": "Sure,I can return the output as a regular JSON object with the key as `text`.Here is an example:{"text":"Your Output"}."}','model'),
